...
def setQuotient(value, limit):
    return limit/value

def padNormalized(read, min, max):
    return (read - min)/(max - min)
...
def getBsonHeight(featureResultNode):
    ...
    height = {
          ...
          'Quotient' : setQuotient(value, maxValue),
        'Normalized' : padNormalized(value, minValue, maxValue)
    }
